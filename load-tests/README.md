...
Эмуляция начала смены: все операторы разом ломятся в `/ws/live`.
0→500 VU за 10 сек → плато 2 мин → обратно за 10 сек → 2 мин наблюдения
за восстановлением (сессии `scada_ws_live_sessions` и heap должны вернуться
к базе).
