...
## Purpose
Фиксация подтвержденных источников данных для фронтенд-контракта и правил агрегации.

## Table of contents
- [Purpose](#purpose)
- [Что это за документ](#что-это-за-документ)
- [Важная оговорка по области применимости](#важная-оговорка-по-области-применимости)
- [Главный вывод](#главный-вывод)
- [Подтвержденные backend-источники для Grunw11](#подтвержденные-backend-источники-для-grunw11)
- [1. REST topology](#1-rest-topology)

## Что это за документ

Документ фиксирует, откуда именно backend может брать данные для контракта из [API_REFERENCE.md](API_REFERENCE.md), если ориентироваться на подтвержденные источники PrintSrv и на карту соответствий из внешнего артефакта `GRUNW11_SCADA_OBJECT_MAPPING.md` (файл не хранится в этом репозитории).

Текущая база анализа:

- автомат `Grunw11`
- frontend-контракт из [API_REFERENCE.md](API_REFERENCE.md)
...
Этот документ нельзя читать как универсальную схему для всех автоматов.

Он построен на детальном анализе именно `Grunw11`, а `Grunw11` в составе устройств заметно богаче многих других автоматов. По реестру автоматов уже видно, что набор устройств в проекте отличается от линии к линии:

- есть автоматы с одним принтером, например `Hassia1`, `Hassia2`
- есть автоматы с двумя принтерами, например `Bosch`, `Grunw1`, `Hassia3`
- есть автоматы с четырьмя принтерами и расширенным набором камер, как `Grunw11`
- есть автоматы с другим типом камер вообще, например `trepko1`, `trepko2`

Из этого следует главное правило интеграции:

- количество камер и принтеров зависит от автомата
- список unit-объектов в PrintSrv зависит от автомата
- набор агрегированных полей в `scada` тоже зависит от автомата

Например, если на линии нет второй камеры агрегации, то backend не должен ожидать наличие `CamAgregation2`, `Dev043...`, `Dev044...` и связанных с ними полей вроде `Dev043CounterGeneral` или `Dev044Error`. Аналогично, если на автомате только один принтер, то не будет ни `Printer12`, ни `LineDev012...`.

Поэтому все примеры ниже надо читать так:

- для `Grunw11` подтверждено именно в таком виде
- для других автоматов backend обязан сначала определить фактический состав устройств, а уже потом строить topology, `DEVICES_STATUS`, `ALERT` и другие ответы
...
- сначала backend определяет, какие устройства реально присутствуют у данного автомата
...
Ниже перечислено то, что подтверждено именно для `Grunw11`. Для другого автомата этот список может быть короче, длиннее или содержать устройства других типов.
...
- это не универсальный список полей `scada` для всех автоматов
- префиксы `Dev041...`, `Dev042...`, `Dev043...`, `Dev044...` появляются только там, где в конфигурации автомата реально есть соответствующие устройства и маппинг в `scada`
