...
## Purpose
Описание протокола PrintSrv, формата данных и доступных команд.

## Table of contents
- [Purpose](#purpose)
- [Общая информация](#общая-информация)
- [Подключение](#подключение)
- [Протокол: формат фрейма](#протокол-формат-фрейма)
- [Иерархия данных](#иерархия-данных)
- [Доступные команды](#доступные-команды)
- [1. QueryAll — Чтение всех тегов](#1-queryall--чтение-всех-тегов)
- [2. SetUnitVars — Установка значений тегов](#2-setunitvars--установка-значений-тегов)
- [Основные теги устройства Line](#основные-теги-устройства-line)
...
| Параметр  | Значение                              |
|-----------|---------------------------------------|
| IP-адрес  | `999.9.9.9` (плейсхолдер в открытом репозитории) или реальный IP сервера |
| Порт      | `9999`                                |
| Протокол  | TCP сокеты                            |
...
> Команды типа `GetUnitVars`, `QueryUnit`, `GetUnit` и т.п. **НЕ существуют** — сервер просто не отвечает на них (
> таймаут).
...
Ответ на `SetUnitVars` возвращает **только изменённые поля** (т.е. те, которые были переданы в запросе). Это **частичный
ответ**, а не полный снимок состояния. Для получения полного состояния используйте `QueryAll`.
...
├──────────────────────────────────────────────────────────────┤
│ Подключение:  999.9.9.9:9999, кодировка windows-1251         │
