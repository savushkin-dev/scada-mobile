...
- [SCADA Mobile](#scada-mobile)
  ...
  - [Состояние проекта (май 2026)](#состояние-проекта-май-2026)
    - [Мобильное направление (TWA/PWA)](#мобильное-направление-twapwa)
  - [Быстрый локальный запуск](#быстрый-локальный-запуск)
    - [Что нужно установить](#что-нужно-установить)
    - [Вариант 1. Запуск по частям (рекомендуемый для разработки)](#вариант-1-запуск-по-частям-рекомендуемый-для-разработки)
    - [Вариант 2. Запуск в Docker (только Prod)](#вариант-2-запуск-в-docker-только-prod)
...
- [backend](backend) — Spring Boot сервис с REST/WebSocket API, JWT-аутентификацией, админ-панелью, health-check и интеграцией с PrintSrv;
- [frontend](frontend) — React + TypeScript + Vite PWA-клиент с React Admin;
- [android](android) — TWA-оболочка для запуска веб-клиента на Android;
- Docker-сценарии для prod запуска уже в репозитории.

### Мобильное направление (TWA/PWA)

- Для TWA и PWA требуется строгое HTTPS-соединение (Service Worker и полноэкранный запуск без адресной строки).
- В корпоративной частной сети получение и обслуживание HTTPS-сертификатов занимает время и организационно сложно.
- Ключевая потребность (уведомления на устройствах с вибрацией) может быть закрыта через веб-клиент в Chrome с Service Worker.
- Поэтому APK/TWA сейчас в статусе потенциальной опции на будущее; детали и сборка описаны в [android/README.md](android/README.md).
...
Запуск: `make docker-prod-up`.

Остановить: `make docker-prod-down`.
