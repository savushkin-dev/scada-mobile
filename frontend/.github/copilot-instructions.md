...
Правила и ориентиры для работы агента с фронтендом проекта.

## Table of contents

- [Purpose](#purpose)
- [1. Что это за приложение](#1-что-это-за-приложение)
- [2. Текущее состояние → целевое](#2-текущее-состояние--целевое)
- [3. Целевая структура (React + Vite)](#3-целевая-структура-react--vite)
- [4. Обязательные файлы PWA/TWA — нельзя удалять или переименовывать](#4-обязательные-файлы-pwatwa--нельзя-удалять-или-переименовывать)
- [5. Доменная модель (данные от бекенда)](#5-доменная-модель-данные-от-бекенда)
- [6. API-интеграция](#6-api-интеграция)
- [7. Service Worker — стратегия кэширования](#7-service-worker--стратегия-кэширования)
- [8. TypeScript — правила](#8-typescript--правила)
- [9. Команды разработки](#9-команды-разработки)
- [10. Что нельзя делать в фронтенде](#10-что-нельзя-делать-в-фронтенде)
