...
При первом запуске, если в базе данных ещё нет пользователей, backend
автоматически создаёт начального администратора со случайно сгенерированным
кодом (логином) и паролем. Учётные данные записываются в файл
`/app/admin-credentials.txt` внутри контейнера backend.
...
> **Важно:** Файл создаётся только при первом старте (когда БД пуста).
> При последующих перезапусках администратор уже существует в БД, и файл
> не обновляется. Сохраните учётные данные надёжно после первого запуска.
