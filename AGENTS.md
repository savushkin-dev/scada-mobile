...
- Offline‑first (CRITICAL): приложение работает в изолированной сети предприятия без доступа в интернет. Никаких внешних CDN — шрифты, стили, скрипты и иконки только локальные. Шрифт Inter (variable) лежит в `frontend/public/assets/fonts/` и подключён через `@font-face` в `frontend/src/index.css`; новые внешние `<link>`/`<script>` на чужие хосты в `index.html` запрещены. Splash‑экран первичной загрузки встроен инлайн в `index.html` (CSS‑спиннер, без SVG) и должен оставаться самодостаточным — ни одного запроса к бандлу или внешним ресурсам.
