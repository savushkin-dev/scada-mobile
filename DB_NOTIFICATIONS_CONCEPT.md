...
erDiagram
  ...
  user_notification_settings {
    ...
    bigint user_id FK
    bigint unit_id FK
    bool system_sound_enabled
    bool system_vibration_enabled
    bool android_push_enabled
    bool is_active
    datetime updated_at
  }
```
...
Таблица Настройки уведомлений пользователя (user_notification_settings) хранит параметры получения уведомлений для конкретного пользователя и автомата. Она определяет, включены ли звук и вибрация для системных событий, и разрешены ли Android push-уведомления. Без нее невозможно централизованно управлять каналами доставки и пользовательскими предпочтениями.
...
- Звук системных уведомлений (`system_sound_enabled`) — включает/выключает звук для карточек событий.
- Вибрация системных уведомлений (`system_vibration_enabled`) — включает/выключает вибрацию для карточек событий.
- Android push (`android_push_enabled`) — разрешает/запрещает отправку Android push-уведомлений.
...
<a name="note1"></a>
**Примечание 1:** Настройки уведомлений пользователя теперь хранятся в БД как единый источник правды. В профиле на фронтенде предусмотрено разграничение: системные события (карточки) и вызовы персонала (Android push).
