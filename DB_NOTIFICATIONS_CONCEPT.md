...
- ID токена (`token_id`) — уникальный идентификатор.
- ID пользователя (`user_id`) — ссылка на пользователя; связь.
